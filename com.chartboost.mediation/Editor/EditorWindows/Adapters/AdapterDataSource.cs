...
namespace Chartboost.Editor.EditorWindows.Adapters
{
    ...
    /// </summary>
    [InitializeOnLoad]
    public static class AdapterDataSource
    {
        private static readonly string PathToLibrary = Path.Combine(Directory.GetCurrentDirectory(), "Library");
        private static readonly string PathToLibraryCacheDirectory = Path.Combine(PathToLibrary, "com.chartboost.mediation");
        private static readonly string PathToAdaptersCachedJson = Path.Combine(PathToLibraryCacheDirectory, "partnersv2.json");
        ...
        {
            if (!PathToLibrary.DirectoryExists())
                return null;

            PathToLibraryCacheDirectory.DirectoryCreate();
            ...
            if (PathToAdaptersCachedJson.FileExist())
            {
                var cachedJson = PathToAdaptersCachedJson.ReadAllText();
                ...
                PathToAdaptersCachedJson.FileCreate(newAdapters);
